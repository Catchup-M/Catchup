...
function CatchupChat({ onBack }) {
  ...
  const toggleEmojiPicker = (e) => {
    ...
    if (willOpen) {
      setTimeout(() => {
        const container = chatContainerRef.current;
        if (container) {
          container.scrollTop = container.scrollHeight - container.clientHeight + emojiPickerHeight;
        }
      }, 350);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col overflow-hidden">
      <div className="w-full max-w-2xl mx-auto bg-white shadow-2xl flex flex-col h-screen overflow-hidden">

        {/* Header */}
        <div className="flex items-center justify-between px-3 py-2 flex-shrink-0 bg-white" onMouseDown={(e) => e.preventDefault()}>
          <button 
            onClick={onBack}
        ...
        <div style={{ boxShadow: '0 1px 2px 0 rgb(0 0 0 / 0.05)', height: '0' }}></div>
        ...
        >
          <div 
            className="sticky top-0 z-10 flex justify-center transition-opacity duration-200 py-1"
          ...
          <div className="flex flex-col justify-end min-h-full space-y-4 py-2">
            ...
            <div ref={messagesEndRef} style={{ marginBottom: '-14px' }} />
          </div>
        </div>
        ...
        {replyingTo && (
          <div className="bg-white px-4 py-2 flex-shrink-0">
            <div 
              className="rounded-lg flex items-center justify-between py-2.5 pl-3 pr-3"
              style={{ 
        ...
        <div className={`bg-white flex-shrink-0 ${!replyingTo ? 'border-t border-gray-200' : ''}`}>
          <div className="w-full max-w-2xl mx-auto flex items-center p-2 gap-1">
            ...
            <button className="p-2 flex-shrink-0 self-end transition-all duration-200" style={{
              opacity: hasText ? 0 : 1,
              transform: hasText ? 'translateX(10px)' : 'translateX(0)',
              pointerEvents: hasText ? 'none' : 'auto',
              width: hasText ? 0 : 'auto',
              padding: hasText ? '0.5rem 0' : '0.5rem'
            }}>
            ...
            <button 
              ...
              style={{
                opacity: hasText ? 1 : 0,
                transform: hasText ? 'translateX(0)' : 'translateX(-10px)',
                pointerEvents: hasText ? 'auto' : 'none',
                width: hasText ? 'auto' : 0,
                padding: hasText ? '0.5rem' : '0.5rem 0'
              }}
            >
        ...
        {/* Emoji Picker */}
        <div
          className="bg-gray-100 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out"
          style={{
            height: showEmojiPicker ? emojiPickerHeight : 0,
          }}
        >
          <div className="w-full max-w-2xl mx-auto h-full p-4">
            <div className="text-gray-500 text-center text-sm">Emoji picker placeholder</div>
          </div>
        </div>
