// chat.js - FINAL VERSION (November 2025) - Perfect Alignment + Your Exact Timestamp Style
...
const MessageBubble = ({ message, setReplyingTo, inputRef }) => {
  ...
  const handleTouchEnd = () => {
    ...
    setIsSwiping(false);
    swipeDirection.current = null;
    if (shouldReply) setReplyingTo(message);
  };
  ...
  return (
    <div className={`flex items-end ${message.isOutgoing ? 'justify-end' : ''}`}>
      ...
      >
        {layout === 'single' ? (
          // SINGLE LINE — Perfect original alignment (unchanged)
          ...
          // FLOW LAYOUT — Your exact requested timestamp style (multi-line only)
          <div className="flex flex-wrap items-end" style={{ wordBreak: 'break-word' }}>
            ...
            {/* YOUR FINAL TIMESTAMP STYLE — ONLY FOR MULTI-LINE */}
            <span ref={timeRef}
                  ...
                  style={{
                    ...
                    color: 'rgb(219, 234, 254)',  // #dbeafe
                    paddingTop: '4px'
                  }}>
              {message.time}
              {message.isOutgoing && <CheckmarkSVG style={{ color: 'rgb(219, 234, 254)' }} />}
            </span>
          </div>
        )}
      </div>
...
function ChatView({ selectedChat = { name: "Alex", avatarImage: "https://via.placeholder.com/40", lastSeen: "online" }, onBack = () => {} }) {
  const [showEmojiPicker, setShowEmojiPicker] = useState(false);
  const [hasText, setHasText] = useState(false);
  const [messages, setMessages] = useState([
    { id: 1, text: 'Afa', time: '07:00 AM', isOutgoing: false },
    { id: 2, text: 'Hello', time: '07:05 AM', isOutgoing: true },
    { id: 3, text: "How are you doing my dear and hope you're doing great with everything going on", time: '11:45 AM', isOutgoing: true },
    { id: 4, text: 'Yes I am good thanks!', time: '11:46 AM', isOutgoing: false },
  ]);
  ...
  const toggleEmojiPicker = (e) => {
    ...
    if (willOpen) {
      setTimeout(() => {
        ...
        if (el) el.scrollTop = el.scrollHeight - el.clientHeight + emojiPickerHeight;
      }, 350);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 flex flex-col">
      ...
      <div className="flex items-center px-3 py-2 bg-white shadow-md">
        ...
        <div className="flex items-center ml-4 flex-1">
          <div className="w-9 h-9 rounded-full overflow-hidden flex-shrink-0">
            <img src={selectedChat.avatarImage} alt={selectedChat.name} className="w-full h-full object-cover" />
          </div>
          <div className="ml-3">
      ...
      {/* Messages Area */}
      <div ref={chatContainerRef} className="flex-1 px-4 bg-white flex flex-col" style={{ overflowX: 'hidden' }}>
        <div className="flex-1 overflow-y-auto flex flex-col scroll-content hide-scrollbar"
             style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          <div className="sticky top-0 z-10 flex justify-center transition-opacity duration-200 py-1"
               style={{ opacity: showFloatingDate ? 1 : 0, pointerEvents: 'none' }}>
          ...
          <div className="flex flex-col flex-grow justify-end">
            ...
            <div className="space-y-3 pb-3">
              {messages.map(msg => (
                <MessageBubble key={msg.id} message={msg} setReplyingTo={setReplyingTo} inputRef={inputRef} />
              ))}
      ...
      {/* Reply Bar */}
      {replyingTo && (
        <div className="bg-white px-4 py-2 flex-shrink-0 border-t border-gray-100">
          <div className="rounded-lg flex items-center justify-between py-2.5 pl-3 pr-3" style={{ backgroundColor: '#f5f5f5', borderLeft: '3px solid #3b82f6' }}>
            ...
            <button onMouseDown={e => e.preventDefault()} onClick={() => setReplyingTo(null)}
      ...
      <div className={`bg-white flex-shrink-0 ${!replyingTo ? 'border-t border-gray-200' : ''}`}>
        <div className="w-full max-w-2xl mx-auto flex items-center p-2 gap-1">
          <button onClick={toggleEmojiPicker} className="p-2 flex-shrink-0 self-end">
            {showEmojiPicker ? (
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 432 384" fill="#6b7280">
                <path d="M384 43q18 0 30.5 12.5T427 85v214q0 17-12.5 29.5T384 341H43q-18 0-30.5-12.5T0 299V85q0-17 12.5-29.5T43 43h341zm-192 64v42h43v-42h-43zm0 64v42h43v-42h-43zm-64-64v42h43v-42h-43zm0 64v42h43v-42h-43zm-21 42v-42H64v42h43zm0-64v-42H64v42h43zm192 150v-43H128v43h171zm0-86v-42h-43v42h43zm0-64v-42h-43v42h43zm64 64v-42h-43v42h43zm0-64v-42h-43v42h43z"/>
              </svg>
            ) : (
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g transform="rotate(-19, 12, 12)">
                  <path d="M9 16C9.85038 16.6303 10.8846 17 12 17C13.1154 17 14.1496 16.6303 15 16" stroke="#6b7280" strokeWidth="2" strokeLinecap="round"/>
                  <ellipse cx="15" cy="10.5" rx="1" ry="1.5" fill="#6b7280"></ellipse>
                  <ellipse cx="9" cy="10.5" rx="1" ry="1.5" fill="#6b7280"></ellipse>
                  <path d="M15 22H12C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.9282 2 7.28595 2 12 2C16.714 2 19.0711 2 20.5355 3.46447C22 4.92893 22 7.28595 22 12V15M15 22C18.866 22 22 18.866 22 15M15 22C15 20.1387 15 19.2081 15.2447 18.4549C15.7393 16.9327 16.9327 15.7393 18.4549 15.2447C19.2081 15 20.1387 15 22 15" stroke="#6b7280" strokeWidth="2"/>
                </g>
              </svg>
            )}
          </button>
          ...
          <div className="flex items-end gap-1 transition-all duration-200 overflow-hidden"
            ...
            <button onMouseDown={e => e.preventDefault()} className="p-2 flex-shrink-0">
              <svg fill="#6b7280" viewBox="0 0 32 32" width="24" height="24">
            ...
            <button onMouseDown={e => e.preventDefault()} className="p-2 flex-shrink-0">
              <svg viewBox="0 0 24 24" fill="none" width="24" height="24">
                <path d="M2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2C16.714 2 19.0711 2 20.5355 3.46447C22 4.92893 22 7.28595 22 12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12Z" stroke="#6b7280" strokeWidth="1.5"/>
            ...
            <button onMouseDown={e => e.preventDefault()} className="p-2 flex-shrink-0 hover:bg-gray-100 rounded-full">
              <svg viewBox="0 0 24 24" fill="none" stroke="#6b7280" strokeWidth="1.8" width="24" height="24">
      ...
      {/* Emoji Picker */}
      <div className="bg-gray-100 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out"
           style={{ height: showEmojiPicker ? emojiPickerHeight : 0 }}>
        <div className="w-full max-w-2xl mx-auto h-full p-4">
          <div className="grid grid-cols-8 gap-2 text-2xl">{"[Emoji Grid Placeholder]"}</div>
        </div>
      </div>
    </div>
  );
...
// Make available globally (great for CodePen, JSFiddle, etc.)
